...
{
    public unsafe sealed class unsafe_dsfmt_t : dsfmt_t_params, Idsfmt
    {
...
#pragma warning restore
        ...
        public void dsfmt_fill_array_close1_open2(double* array, int size)
        {
            ...
            gen_rand_array_c1o2((w128_t*)array, size / 2);
        }

        public void dsfmt_fill_array_close1_open2(double[] array, int size)
        {
            fixed (double* p = array) {
                this.dsfmt_fill_array_close1_open2(p, size);
            }
        }
        ...
        public void dsfmt_fill_array_open_close(double* array, int size)
        {
            assert(size % 2 == 0);
            assert(size >= DSFMT_N64);
            gen_rand_array_o0c1((w128_t*)array, size / 2);
        }

        public void dsfmt_fill_array_open_close(double[] array, int size)
        {
            fixed (double* p = array) {
                this.dsfmt_fill_array_open_close(p, size);
            }
        }
        ...
        public void dsfmt_fill_array_close_open(double* array, int size)
        {
            assert(size % 2 == 0);
            assert(size >= DSFMT_N64);
            gen_rand_array_c0o1((w128_t*)array, size / 2);
        }

        public void dsfmt_fill_array_close_open(double[] array, int size)
        {
            fixed (double* p = array) {
                this.dsfmt_fill_array_close_open(p, size);
            }
        }
        ...
        public void dsfmt_fill_array_open_open(double* array, int size)
        {
            assert(size % 2 == 0);
            assert(size >= DSFMT_N64);
            gen_rand_array_o0o1((w128_t*)array, size / 2);
        }

        public void dsfmt_fill_array_open_open(double[] array, int size)
        {
            fixed (double* p = array) {
                this.dsfmt_fill_array_open_open(p, size);
            }
        }
        ...
        {
            fixed (w128_t* status = this.status) {
                var psfmt = &status[0].u32_0;
                psfmt[0] = seed;
                for (uint i = 1; i < (DSFMT_N + 1) * 4; i++) {
                    psfmt[i] = 1812433253U * (psfmt[i - 1] ^ (psfmt[i - 1] >> 30)) + i;
                }
            }
            initial_mask();
        ...
        void dsfmt_chk_init_by_array(uint32_t[] init_key)
        {
            fixed (uint32_t* arr = init_key) {
                dsfmt_chk_init_by_array(arr, (uint)init_key.Length);
            }
        }
        ...
        public double dsfmt_genrand_open_open()
        {
            ...
            fixed (w128_t* status = this.status) {
                double* dsfmt64 = &status[0].d0;
                double d = dsfmt64[idx++];
                *(uint64_t*)&d |= 1;
                return d - 1.0;
            }
        }
        ...
        {
            fixed (uint32_t* arr = init_key) {
                dsfmt_init_by_array(arr, (uint)init_key.Length);
            }
        }
    }
}
